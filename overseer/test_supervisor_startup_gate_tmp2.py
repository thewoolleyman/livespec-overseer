...
def test_tmux_column_annotates_a_claude_row_with_its_runtime(*, tmp_path):
    ...
    line = cell_row(out=render_of(sup=sup, views=[view]), topic="wk")
...
def test_tmux_column_annotates_a_codex_row_with_its_runtime(*, tmp_path):
    ...
    line = cell_row(out=render_of(sup=sup, views=[view]), topic="cx")
...
def test_tmux_column_is_a_bare_dash_with_no_runtime_for_no_pane_rows(*, tmp_path):
    ...
    for topic, status in (("un", "unassigned"), ("sg", "session-gone")):
        ...
        line = cell_row(out=render_of(sup=sup, views=[view]), topic=topic)
        assert "—" in line
...
def test_tmux_runtime_annotation_preserves_column_alignment(*, tmp_path):
    ...
    wide = cell_row(out=out, topic="alpha")  # tmux cell "livespec (claude)" sets the column width
    narrow = cell_row(out=out, topic="beta")  # tmux cell "—" padded to that same width
