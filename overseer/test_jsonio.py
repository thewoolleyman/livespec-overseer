...
def test_as_list_narrows_a_list():
    assert jsonio.as_list(value=[1, "two", None]) == [1, "two", None]


def test_as_list_preserves_the_empty_list():
    """`[]` is falsy — it must be distinguishable from "not a list"."""
    assert jsonio.as_list(value=[]) == []
    assert jsonio.as_list(value=[]) is not None
...
def test_as_float_returns_zero_not_none_for_zero():
    ...
    assert jsonio.as_float(value=0) == 0.0
    assert jsonio.as_float(value=0) is not None
