...
def test_live_codex_session_joins_pid_to_its_thread_name_and_cwd(*, tmp_path):
    ...
    home = fake_index(tmp_path=tmp_path, records=[(ID_A, "rop-sweep-consumer-cleanup")])
    host = fake_host(
        comms={4242: "codex"},
        cwds={4242: "/data/projects/livespec"},
        fds={4242: ["/dev/null", fake_rollout(session_id=ID_A), "/some/other/file"]},
    )
...
def test_non_codex_processes_are_ignored(*, tmp_path):
    ...
    home = fake_index(tmp_path=tmp_path, records=[(ID_A, "some-topic")])
    host = fake_host(
        comms={1: "bun", 2: "node", 3: "zsh"},
        cwds={1: "/data/projects/livespec", 2: "/x", 3: "/y"},
        fds={1: [fake_rollout(session_id=ID_A)]},  # even if it somehow held one
    )
...
def test_unnamed_session_is_skipped(*, tmp_path):
    ...
    host = fake_host(
        ...
        fds={9: [fake_rollout(session_id=ID_B)]},  # live, but its id is NOT in the index
    )
    assert codex_sessions.read_live_codex_sessions(codex_home=home, **host) == []
...
def test_companion_task_threads_are_returned_not_filtered_here(*, tmp_path):
    ...
    home = fake_index(tmp_path=tmp_path, records=[(ID_A, "Codex Companion Task: do a thing")])
    host = fake_host(
        comms={5: "codex"},
        cwds={5: "/data/projects/livespec"},
        fds={5: [fake_rollout(session_id=ID_A)]},
    )
    out = codex_sessions.read_live_codex_sessions(codex_home=home, **host)
...
def test_a_process_with_no_readable_cwd_is_skipped(*, tmp_path):
    ...
    never raised."""
    home = fake_index(tmp_path=tmp_path, records=[(ID_A, "topic")])
    host = fake_host(comms={5: "codex"}, cwds={}, fds={5: [fake_rollout(session_id=ID_A)]})
    assert codex_sessions.read_live_codex_sessions(codex_home=home, **host) == []
...
def test_multiple_live_sessions_all_join(*, tmp_path):
    ...
    host = fake_host(
        ...
        fds={11: [fake_rollout(session_id=ID_A)], 12: [fake_rollout(session_id=ID_B)]},
    )
    out = codex_sessions.read_live_codex_sessions(codex_home=home, **host)
...
def test_index_last_record_wins_for_a_repeated_id(*, tmp_path):
    ...
    home = fake_index(tmp_path=tmp_path, records=[(ID_A, "old-name"), (ID_A, "new-name")])
...
def test_latest_session_for_thread_name_is_none_for_an_unknown_topic(*, tmp_path):
    ...
    home = _index_ts(
        tmp_path=tmp_path, records=[(ID_A, "some-codex-topic", "2026-07-13T10:00:00Z")]
    )
    assert (
...
def test_rollout_exists_finds_a_nested_rollout(*, tmp_path):
    home = tmp_path / "codex"
    home.mkdir()
    _write_rollout(home=home, session_id=ID_A)
...
def test_rollout_exists_is_false_when_the_rollout_is_gone(*, tmp_path):
    ...
    home = tmp_path / "codex"
    home.mkdir()
    _write_rollout(home=home, session_id=ID_A)  # a DIFFERENT session's rollout is present
...
def test_rollout_id_is_read_from_the_filename(*, tmp_path):
    assert codex_sessions.rollout_id(path=fake_rollout(session_id=ID_A)) == ID_A
...
def test_open_rollout_id_picks_the_rollout_out_of_unrelated_fds():
    fds = [
        "/dev/urandom",
        "/home/u/.codex/logs_2.sqlite-wal",
