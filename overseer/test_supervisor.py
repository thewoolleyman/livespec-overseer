...
def test_blocked_marker_suppresses_injection(tmp_path):
    ...
    fake = FakeTmux()
    fake.serve(session, repo, capture=idle_capture(ctx=40))  # idle+low ctx but blocked marker
    sup = make_supervisor(tmp_path, fake)
    view = sup.evaluate(mapped_track(repo, topic, session), act=True)
    assert view.status == "blocked:human"
...
def test_shell_pane_never_pastes(tmp_path):
    ...
    fake.serve(session, repo, capture=idle_capture(ctx=40), cmd="zsh")
...
def test_a_foreign_pane_is_session_gone_not_a_status_of_its_own(tmp_path):
    ...
    session = registry.tmux_id(str(repo), topic)
    fake = FakeTmux()
    fake.serve(session, other, capture=idle_capture(ctx=40))  # live claude, wrong repo
    sessions_dir = tmp_path / "sessions"
    sessions_dir.mkdir()
    sup = adopt_sup(tmp_path, fake, sessions_dir, {}, {})
    view = sup.evaluate(mapped_track(repo, topic, session), act=True)
...
def test_identity_rechecked_before_acting_catches_shell(tmp_path):
    ...
    session = registry.tmux_id(str(repo), topic)
    fake = FakeTmux()
    fake.serve(session, repo, capture=idle_capture(ctx=40))  # idle, low ctx → would inject
...
def test_warned_writes_stamp_before_pasting(tmp_path):
    ...
    session = registry.tmux_id(str(repo), topic)
    fake = FakeTmux()
    fake.serve(session, repo, capture=idle_capture(ctx=40))  # below the default warn threshold (50)
...
def test_void_resets_inject_state_so_round_can_recertify(tmp_path):
    ...
    fake.serve(session, repo, capture=idle_capture(ctx=40))
    sup.evaluate(track, act=True)
    assert key_for(repo, topic) in sup._inject  # in-memory last_ctx tracked
    ...
    sup.evaluate(track, act=True)
    assert key_for(repo, topic) not in sup._inject  # inject state popped
...
def test_missing_session_without_any_live_claude_is_still_session_gone(tmp_path):
    ...
    write_session(sessions_dir, 100, name="some-other-topic", cwd=str(repo), status="busy")
    sup = adopt_sup(tmp_path, fake, sessions_dir, {}, {100: "pt"})
    view = sup.evaluate(mapped_track(repo, topic, session), act=True)
    assert view.status == "session-gone"
...
def test_missing_session_with_the_claude_in_a_different_tmux_is_session_gone(tmp_path):
    ...
    sessions_dir.mkdir()
    write_session(sessions_dir, 100, name=topic, cwd=str(repo), status="busy")
    sup = adopt_sup(tmp_path, fake, sessions_dir, {100: 4242}, {100: "pt"})
    view = sup.evaluate(mapped_track(repo, topic, session), act=True)
    assert view.status == "session-gone"
...
def test_tty_render_leaves_live_outside_tmux_uncolored(tmp_path):
    ...
    line = row_line(render_of(sup, [view]), "lo")
...
def test_absent_proc_is_an_unsupported_host(tmp_path):
    ...
    sup = make_supervisor(tmp_path, FakeTmux(), proc_root=str(tmp_path / "no-such-proc"))
...
def test_absent_tmux_is_an_unsupported_host(tmp_path):
    ...
    sup = make_supervisor(tmp_path, FakeTmux(), which=lambda _name: None)
    reasons = sup.unsupported_host_reasons()
    assert len(reasons) == 1
...
def test_the_gate_asks_about_tmux_by_its_real_name(tmp_path):
    ...
    asked: list[str] = []
    sup = make_supervisor(
        tmp_path, FakeTmux(), which=lambda name: asked.append(name) or "/usr/bin/tmux"
    )
...
def test_run_refuses_on_an_unsupported_host_before_ticking(tmp_path):
    ...
    repo, _topic = make_plan(tmp_path)
    err = _io.StringIO()
    sup = make_supervisor(
        tmp_path,
        FakeTmux(),
...
def test_run_refuses_when_tmp_not_gitignored(tmp_path):
    ...
    repo, _topic = make_plan(tmp_path)
    fake = FakeTmux()
    sup = make_supervisor(tmp_path, fake, watch_repos=[str(repo)], gitignore_check=lambda _r: False)
...
def test_run_proceeds_when_tmp_gitignored(tmp_path):
    ...
    repo, _topic = make_plan(tmp_path)
    fake = FakeTmux()
    sup = make_supervisor(tmp_path, fake, watch_repos=[str(repo)], gitignore_check=lambda _r: True)
...
def test_build_rows_caches_the_cross_repo_collision_set(tmp_path):
    ...
    r1, _ = make_plan(tmp_path, repo_name="livespec", topic="shared")
    make_plan(tmp_path, repo_name="livespec", topic="solo-a")
    r2, _ = make_plan(tmp_path, repo_name="other", topic="shared")
    make_plan(tmp_path, repo_name="other", topic="solo-b")
...
def test_tmux_column_annotates_a_claude_row_with_its_runtime(tmp_path):
    ...
    line = _cell_row(render_of(sup, [view]), "wk")
...
def test_tmux_column_annotates_a_codex_row_with_its_runtime(tmp_path):
    ...
    line = _cell_row(render_of(sup, [view]), "cx")
...
def test_tmux_column_is_a_bare_dash_with_no_runtime_for_no_pane_rows(tmp_path):
    ...
    for topic, status in (("un", "unassigned"), ("sg", "session-gone")):
        ...
        line = _cell_row(render_of(sup, [view]), topic)
        assert "—" in line
...
def test_two_codex_tracks_sharing_a_tmux_session_each_restart_their_own_session(tmp_path):
    ...
    repo, topic_a = make_plan(tmp_path, topic="alpha")
...
def test_codex_restart_alerts_when_the_codex_session_vanished_before_the_respawn(tmp_path):
    ...
    with contextlib.redirect_stderr(err):
        sup._do_codex_restart(mapped_track(repo, topic, session), session)
