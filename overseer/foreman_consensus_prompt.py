...
def reviewer_action_contract(*, action_ids: frozenset[str] | None = None) -> str:
    ...
    return (
        ...
        "action must be an object. For an action member, action_id must be one of: "
        f"{', '.join(sorted(allowed))}. params must be a JSON object. "
        "For a typed ruling member, set member_kind to typed_ruling and provide "
        "ruling as a JSON object carrying kind plus every field the executing "
        "session needs. "
