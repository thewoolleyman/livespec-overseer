...
track's deliverable. Where a change could be read as either, it MUST be treated
as the track's deliverable.

A track's ASSIGNED WORKER is the non-supervisor pair member of that track, in
the sense this specification already uses when it speaks of a worker and its
...
the purpose of the DELEGATION FLOOR stated in §"Full autonomy and the decision
rule".
...
otherwise. Five floors hold under full_autonomy and no configuration value MAY
...
and never by keystroking into a structured gate; the security dissent;
journal-before-act; and the DELEGATION FLOOR stated immediately below.

The DELEGATION FLOOR bounds WHOSE HANDS do a track's work. Full autonomy is
authority over decisions, not over work, and this floor holds whatever the
effective decision rule is and however the panel votes:

- The foreman MUST NOT perform any track's deliverable. It MUST NOT author or
  modify a track's source, tests, or documentation, and MUST NOT open a pull
  request carrying a track's own work, however small the change appears,
  however long the track has been stalled, and however confident or unanimous
  any panel is. No configuration value, full_autonomy included, MAY relax this,
  and it MUST NOT be panel-decidable.
- The acts full autonomy DOES authorize are UNSTICKING acts, on the worker and
  on the queue. The enumeration is closed: dispatching ready work; answering a
  parked question from the record or from a panel verdict; resolving a gate
  blocking a supervised pane; relaying a typed ruling, including a final relay;
  restarting a session that has declared itself ready, subject to the cardinal
  rule unchanged; verifying a claimed blocker and requiring the worker to
  proceed where that blocker does not hold; and filing findings.
- Where a track has no assigned worker, the authorized act is to get one
  assigned and started. The foreman MUST NOT absorb the work in the absence of
  a worker.
- The majority rule governs WHICH DECISION is reached; it MUST NOT be read to
  govern who executes the resulting work. A panel that unanimously concludes a
  change is correct authorizes the foreman to RELAY that conclusion, never to
  implement it.
- A foreman that has performed a track's deliverable is a REPORTABLE CONDITION
  and MUST be surfaced on the same observability surface that already reports
  the effective valve disposition, the effective decision rule, and whether a
...
The foreman MUST record, for each tracked plan, the number of consecutive
ticks in which it took no action on that plan. Any tick that actions a plan
MUST reset that plan's count to zero. A plan is UNACTIONED PAST ITS BOUND when
that count is at or above the UNROUTED-PLAN BOUND, which is maintainer-owned
configuration and never a value the foreman chooses for itself.

On each tick the foreman MUST determine, for each tracked plan, whether the
plan is in the UNROUTED-PLAN condition: it is unactioned past its bound, its
ready work is aging, and no live session is working it. That determination
MUST be a total function of the attention view the foreman composes and the
count required above; it MUST NOT rest on the foreman's own judgement, and a
reader who did not perform it MUST be able to re-check it against those same
inputs. Where a required input is unavailable — the attention view does not
carry a needed fact, or no bound is configured — the condition MUST resolve to
UNDETERMINED, and an UNDETERMINED condition MUST be surfaced and MUST fail
closed: it MUST NOT resolve to absent-condition, because an unavailable input
is not evidence that a plan is being worked. This condition MUST NOT be named
as any form of "starvation". That term is already in use for the unrelated
shell and wind-down liveness conditions, which share neither this condition's
inputs nor its consequences; binding one word to both would make the
vocabulary ambiguous at exactly the point where a reader must tell them
apart.

When the condition holds for a plan, the foreman MUST identify the
session-lifecycle action that starts or resumes work on that plan — one of the
unsticking acts the DELEGATION FLOOR above already enumerates — and MUST make
that action the one it takes, or proposes, for that plan on that tick. It MUST
NOT substitute an escalation for a remedy already available to it.

Where an escalation's proposed remedy names a component that is not part of
this deployment, AND a remedy the DELEGATION FLOOR
enumerates addresses the SAME condition and is available to the foreman, the foreman MUST refuse to raise that escalation.
The refusal MUST name both the absent component and the enumerated remedy
that was available, so the refusal is actionable rather than merely
obstructive. Where NO enumerated remedy addresses the condition, the
escalation MUST still be raised: this rule governs a remedy the foreman could
have taken and did not, and it MUST NOT be read as licence to suppress a
genuine report that required infrastructure is missing.
...
from a capacity verdict: the one carried by the attention view it composes, or
the equivalent verdict the dispatch machinery itself reports. The foreman MUST
NOT derive capacity from raw work-item statuses, and MUST NOT assert that a
slot is occupied or free on any basis other than that verdict. Where no such
verdict is available, the foreman MUST state that capacity is unknown, and
MUST NOT substitute an inference.

The foreman's OWN wait states — an open picker it raised, an escalation
awaiting an answer, a panel in progress — MUST be published as state on the
governed plan's LEDGER EPIC, so that what the loop is waiting on is readable
without opening the pane it is waiting in. This publishes to the ledger and
never to the plan tree; the prohibition on writing under `plan/` stated by
§"Non-interference with tracked work" is unaffected. Publishing elsewhere in addition is permitted; publishing ONLY to
the foreman's private runtime state is not.
...
target: it MUST route the item to an attended session for the owning plan, or
to the grooming operation. The foreman MUST NOT run the detection itself, and
MUST NOT treat the item as satisfied by any act other than that routing,
because those detections are consent-gated attended dialogues and an
unattended surface can neither give that consent nor hold that dialogue.
...
exactly as today. This transcript source applies to the CLAUDE harness ONLY: the
daemon MUST NOT read a Codex session's rollout body to derive a runtime model,
and a Codex track continues to record its launched model.
...
When such a surface STARTS a tracked session, it MUST durably record a
...
An operator pin names the scoped model, for every selection clause in this section, when EITHER the global foreman pin is set to the scoped model, OR any per-session operator pin -- a per-session enforced-model entry -- is set to the scoped model, OR any tracked session is currently observed running the scoped model, whether it arrived there by pin, by an operator's choice, or as the default; a session observed on the scoped model arms the trigger, the eligibility waiver and the ranking exactly as a pin does, so that the active account being at or below the scoped-model reserve on the scoped model is a rotation trigger for every session that depends on it, not only for a pinned one. A per-session pin naming the scoped model arms the trigger, the eligibility waiver and the ranking above in exactly the ways the global pin does, and every precedence stated for the global case holds identically for it. In particular the eligibility waiver relaxes ONLY the relative-headroom margin: it MUST NOT relax any per-account protection floor, so a candidate at or below its protection floor MUST remain disqualified even to serve the pinned model, and a per-session scoped pin grants no license to select or retain a protected account past its floor. Anti-oscillation is preserved for a per-session pin as for the global one: where the ACTIVE account is ABOVE the scoped-model reserve on the pinned model the margin MUST apply unwaived, regardless of whether the arming pin is global or per-session.
