...
exactly as before — the floor is CLEARED when the round closes — at restart,
or at a recovered-round closure — not raised, and plays no part.
...
- **The round sidecar** (`~/.livespec-overseer-stamps.json`) — a JSON object
  ...
  bands; the RESTART and the RECOVERED-ROUND CLOSURE each delete the key
  entirely, so no round datum — floor and identity included — outlives its
  round. Key deletion is tied to those two round-closing events specifically
  rather than to any other round event: a voided declaration leaves the key
  in place. A legacy
