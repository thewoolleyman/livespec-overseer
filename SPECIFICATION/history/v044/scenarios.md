...
## Scenario: A foreman tick that ends with its own blocking prompt outstanding is a reportable violation

Given the foreman has raised its own blocking question for a human decision it cannot make itself

When the tick ends with that question still outstanding

Then the tick is reported as a violation of the non-blocking escalation requirement

And it is not treated as a permitted bounded wait

And no restart authorization is altered by the violation or by its reporting
...
When the panel's verdicts fail the effective decision rule, a reviewer verdict that rule treats as a veto is present, the evidence is unavailable, or the journal append fails
...
## Scenario: A needs-human dissent without a security risk kind is outvoted under the majority rule

Given a repository whose livespec configuration declares full_autonomy true

And a human valve whose category sits below no floor

When one reviewer returns needs-human carrying either no hard-risk marker or a hard-risk marker whose declared risk kind is other, and the other two return the same typed action

Then the foreman appends an audit journal entry recording a majority outcome and the dissent
...
And does not declare full_autonomy true
...
And full_autonomy resolves to false
...
And the malformed record is skipped and surfaced

## Scenario: An expired wait premise is not presented as current evidence
...
## Scenario: A panel-authorized change is relayed to the worker rather than implemented by the foreman

Given full autonomy resolves true

And a constituted panel authorizes a typed action carrying a code change

When the foreman acts on that verdict

Then the foreman relays the ruling to the track's assigned worker

And no track deliverable is authored by the foreman seat

## Scenario: A track with no assigned worker resolves to an assignment rather than to foreman-authored work

Given full autonomy resolves true

And a ready track carries no assigned worker

When the foreman acts

Then the foreman assigns and starts a worker for that track

And the foreman does not author the track's deliverable

## Scenario: The delegation floor holds against a unanimous panel

Given full autonomy resolves true

And every reviewer agrees the foreman should make a one-line fix directly

When the verdict is evaluated

Then the act is refused as a floor violation rather than authorized
...
## Scenario: An unrouted plan yields the enumerated remedy as the foreman's own action
...
Then the unrouted-plan condition is determined from the attention view and the
consecutive-unactioned count

And the foreman identifies the session-lifecycle action that starts or resumes
work on that plan

And that action is the one the foreman takes or proposes for that plan on that
tick

And no escalation is substituted for a remedy already available to it

## Scenario: A tick that actions a plan resets that plan's consecutive-unactioned count

Given a tracked plan whose consecutive-unactioned count is at or above the
unrouted-plan bound

When the foreman ticks and takes an action on that plan

Then that plan's consecutive-unactioned count is reset to zero

And the plan is not in the unrouted-plan condition on a later tick on the
strength of the count it carried before that action

## Scenario: A missing required input yields undetermined, never absent-condition

Given a required input for the unrouted-plan condition is unavailable

When the foreman ticks

Then the condition resolves to undetermined for that plan

And the missing input is surfaced

And the plan is not reported as being worked on the strength of the absent
fact
...
And a proposed escalation whose remedy names a component this deployment does
not have

And a remedy the DELEGATION FLOOR enumerates is available to the foreman for
that same condition

When the foreman evaluates that escalation

Then the escalation is refused rather than raised

And the refusal names the absent component and the available enumerated
remedy

## Scenario: A genuine report of missing infrastructure is still raised

Given a proposed escalation whose remedy names a component this deployment
does not have

And no enumerated remedy addresses that condition
...
Given an attention view carrying a capacity verdict
...
## Scenario: Capacity with no available verdict is stated as unknown, never inferred

Given no capacity verdict is available from the attention view or from the
dispatch machinery

When the foreman authors a tick report, an escalation, or a panel dossier

Then it states that capacity is unknown

And it does not assert that a slot is occupied or free

And no capacity claim is derived from raw work-item statuses

## Scenario: A foreman wait is readable without opening its pane

Given the foreman is waiting on an open picker, an escalation, or a panel in
progress

When a reader consults the governed plan's ledger epic

Then the wait is published there as state
...
Given the attention view surfaces a detection-staleness item

When the foreman ticks

Then the item is routed to an attended session for the owning plan or to the
grooming operation
