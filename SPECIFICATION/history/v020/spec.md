...
the stall from the recipient to the sender. This floor governs delivery
routing only; like the escalation floor above, it MUST NOT be read to alter,
in any way, what may authorize a restart of a tracked session — the cardinal
rule, that a session is restarted only when it declares itself ready on the
filesystem, is unaffected by it.
