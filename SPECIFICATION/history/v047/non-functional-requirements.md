...
- The standard-library-only rule (per constraints.md §"Language and
  ...
  launch mode, which would fail on any third-party import from an installed
  environment. A library vendored in-tree under the exemption in that same
  section is by construction not such an import, and does not fail it.
