...
The ratified text names exactly one event that closes a supervision round — the restart — but the daemon also closes a round when it voids a stale `ready`, deleting the round's durable key and resetting its notified escalation bands. This finding states, normatively, that voiding a declaration clears the DECLARATION ONLY and MUST NOT delete the round's durable record or reset its bands. It is a DIVERGENCE from already-ratified text and needs no new contract; it is filed first because the two findings that follow both depend on the round record surviving a void.
...
Measured consequence, from tmp/overseer/daemon.log on the livespec-overseer `foreman` track, 2026-08-03: 41 wrap-up injections and 39 voids alternated on a roughly two-and-a-half-minute period between 06:57:00Z and 08:52:59Z, with zero restarts ever recorded for that track. The reported band sets SHRINK across those injections — 23 occurrences of [50, 40, 30], 8 of [50, 40, 30, 20], 6 of [50, 40], 4 of [50] — and a band set cannot shrink within one round, because bands are append-only per round. That is direct evidence the key was deleted and re-accumulated dozens of times. Two defects follow. First, the spam-proofing that spec.md §"The escalating wrap-up" ratifies is defeated: a session that oscillates between declaring and resuming work is re-warned indefinitely, roughly every two and a half minutes, because each void manufactures a fresh round. Second, and the reason this thread exists, the round record needed to certify a later declaration is destroyed.

This also corrects a premise this project reasoned from for a time: the failure was believed to be BAND EXHAUSTION, a round whose bands had all fired and could not re-arm. The measurement above shows the opposite — bands reset on every void and were never exhausted.
