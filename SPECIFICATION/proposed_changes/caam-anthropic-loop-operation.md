...
spec_commitments:
  impl_followups:
    ...
    - id_hint: caam-keep-profiles-warm
      description: |
        Implement active maintenance of the verifiable-account set: refresh a non-active account's stored credential before its short-lived token lapses, by exercising it through the agent in an isolated configuration sandbox rather than by calling the token endpoint, with a staleness margin, a per-account retry backoff, orphan reporting, and a verified-unchanged live credential. Carrier group X of plan/caam-anthropic-loop/research/feature-inventory.md. Must be implemented together with the live-verified selection rule (carrier G8); implementing that rule alone reproduces the deadlock this fixes.
