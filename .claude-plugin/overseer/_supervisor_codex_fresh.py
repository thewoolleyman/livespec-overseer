...
def _named_and_adopted(
    *,
    ...
    """Submit ``/rename <topic>``, then wait for the adoption join to report the successor.

    The confirmation is the ADOPTION JOIN, not the pane going busy: a slash command asks
    the model for nothing, so Codex's usual submit-confirm signal never fires for it.
    Waiting on the join instead is the stronger check anyway — it is exactly the question
    the next tick will ask. Enter is re-sent while the poll runs, bounded by the same
    ``SUBMIT_MAX_ENTERS`` budget the prompt submitter uses, because a freshly-respawned
    TUI can drop the first one; an extra Enter on an empty composer is a no-op.
    """
