---
name: livespec-overseer-grooming
description: Run the bounded livespec grooming drain pass for the current repository.
allowed-tools: bash read
---

# livespec-overseer-grooming - pi binding

This file is the thin pi binding of the `grooming` operation of the
**livespec-overseer** plugin. It carries pi-runtime mechanics ONLY. The
complete drain-pass contract is the plugin's shared artifact at
`prose/grooming.md`, the same artifact the Claude and Codex bindings read.

Order of work, every time:

1. Resolve `$PLUGIN_ROOT` (next section).
2. Read `$PLUGIN_ROOT/prose/grooming.md` **completely** with the `read` tool.
...
rather than by the `/livespec-overseer:grooming` form the Claude and Codex
surfaces use.
