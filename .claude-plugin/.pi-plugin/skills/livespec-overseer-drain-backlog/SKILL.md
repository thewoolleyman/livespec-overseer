---
name: livespec-overseer-drain-backlog
description: Drain a livespec repository's open backlog through the factory in ruled batches.
allowed-tools: bash read
---

# livespec-overseer-drain-backlog - pi binding

This file is the thin pi binding of the `drain-backlog` operation of the
**livespec-overseer** plugin. It carries pi-runtime mechanics ONLY. The complete
operator contract is the plugin's shared artifact at
`prose/drain-backlog.md`, the same artifact the Claude and Codex bindings
read.

Order of work, every time:

1. Resolve `$PLUGIN_ROOT` (next section).
2. Read `$PLUGIN_ROOT/prose/drain-backlog.md` **completely** with the
   `read` tool.
3. Execute that prose end-to-end, binding its harness-neutral vocabulary to this
   runtime via the Runtime bindings section below.

Never paraphrase, summarize, or act on a partial read of the prose, and never
restate its steps here - the prose owns the behavior, this file owns the wiring.
...
rather than by the `/livespec-overseer:drain-backlog` form the Claude and
Codex surfaces use.
