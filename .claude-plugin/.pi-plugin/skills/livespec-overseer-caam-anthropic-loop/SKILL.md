---
name: livespec-overseer-caam-anthropic-loop
description: Watch caam-managed Claude Max account usage and rotate safely.
allowed-tools: bash read
---

# livespec-overseer-caam-anthropic-loop - pi binding

This file is the thin pi binding of the `caam-anthropic-loop` operation of the
...
`prose/caam-anthropic-loop.md`, the same artifact the Claude and Codex bindings
read.

Order of work, every time:

1. Resolve `$PLUGIN_ROOT` (next section).
2. Read `$PLUGIN_ROOT/prose/caam-anthropic-loop.md` **completely** with the
   `read` tool.
...
rather than by the `/livespec-overseer:caam-anthropic-loop` form the Claude and
Codex surfaces use.
