...
Thin Codex binding for the `foreman` operation of the
**livespec-overseer** plugin. The behavior lives in the plugin-owned prose
artifact at `prose/foreman.md`; this binding only resolves the plugin root and
reads that contract.
...
if [ -z "$PLUGIN_ROOT" ]; then
  ...
  if [ -d "$CODEX_CACHE_PARENT" ]; then
    ...
    if [ -n "$CANDIDATE_PLUGIN_ROOT" ] && [ -f "$CANDIDATE_PLUGIN_ROOT/prose/foreman.md" ]; then
      PLUGIN_ROOT="$CANDIDATE_PLUGIN_ROOT"
    else
...
if [ -z "$PLUGIN_ROOT" ] || [ ! -f "$PLUGIN_ROOT/prose/foreman.md" ]; then
...
Read `$PLUGIN_ROOT/prose/foreman.md` in full, then execute it end-to-end.

```bash
cat "$PLUGIN_ROOT/prose/foreman.md"
```
