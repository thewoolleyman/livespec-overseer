---
name: drain-backlog
description: Drain a livespec repository's open backlog through the factory in ruled batches. Invoked as livespec-overseer:drain-backlog.
---

# drain-backlog - Codex binding

Thin Codex binding for the `drain-backlog` operation of the
**livespec-overseer** plugin. The behavior lives in the plugin-owned prose
artifact at `prose/drain-backlog.md`; this binding only resolves the
...
if [ -z "$PLUGIN_ROOT" ]; then
  ...
  if [ -d "$CODEX_CACHE_PARENT" ]; then
    ...
    if [ -n "$CANDIDATE_PLUGIN_ROOT" ] && [ -f "$CANDIDATE_PLUGIN_ROOT/prose/drain-backlog.md" ]; then
      PLUGIN_ROOT="$CANDIDATE_PLUGIN_ROOT"
    else
...
if [ -z "$PLUGIN_ROOT" ] || [ ! -f "$PLUGIN_ROOT/prose/drain-backlog.md" ]; then
...
Read `$PLUGIN_ROOT/prose/drain-backlog.md` in full, then execute it
end-to-end.

```bash
cat "$PLUGIN_ROOT/prose/drain-backlog.md"
```
