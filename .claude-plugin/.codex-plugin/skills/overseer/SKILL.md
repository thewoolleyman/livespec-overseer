---
name: overseer
description: Start and operate the livespec two-pane overseer surface. Invoked as livespec-overseer:overseer.
---

# overseer - Codex binding

Thin Codex binding for the `overseer` operation of the
**livespec-overseer** plugin. The behavior lives in the plugin-owned prose
artifact at `prose/overseer.md`; this binding only resolves the plugin root
and reads that contract.
...
if [ -z "$PLUGIN_ROOT" ]; then
  ...
  if [ -d "$CODEX_CACHE_PARENT" ]; then
    ...
    if [ -n "$CANDIDATE_PLUGIN_ROOT" ] && [ -f "$CANDIDATE_PLUGIN_ROOT/prose/overseer.md" ]; then
      PLUGIN_ROOT="$CANDIDATE_PLUGIN_ROOT"
    else
...
if [ -z "$PLUGIN_ROOT" ] || [ ! -f "$PLUGIN_ROOT/prose/overseer.md" ]; then
...
Read `$PLUGIN_ROOT/prose/overseer.md` in full, then execute it end-to-end.
