...
description: Watch caam-managed Claude Max account usage and rotate safely. Invoked as livespec-overseer:caam-anthropic-loop.
---

# caam-anthropic-loop - Codex binding

Thin Codex binding for the `caam-anthropic-loop` operation of the
**livespec-overseer** plugin. The behavior lives in the plugin-owned prose
artifact at `prose/caam-anthropic-loop.md`; this binding only resolves the
plugin root and reads that contract.
...
if [ -z "$PLUGIN_ROOT" ]; then
  ...
  if [ -d "$CODEX_CACHE_PARENT" ]; then
    ...
    if [ -n "$CANDIDATE_PLUGIN_ROOT" ] && [ -f "$CANDIDATE_PLUGIN_ROOT/prose/caam-anthropic-loop.md" ]; then
      PLUGIN_ROOT="$CANDIDATE_PLUGIN_ROOT"
    else
...
if [ -z "$PLUGIN_ROOT" ] || [ ! -f "$PLUGIN_ROOT/prose/caam-anthropic-loop.md" ]; then
...
Read `$PLUGIN_ROOT/prose/caam-anthropic-loop.md` in full, then execute it
end-to-end.

```bash
cat "$PLUGIN_ROOT/prose/caam-anthropic-loop.md"
```
