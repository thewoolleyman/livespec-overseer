...
| `overseer-jcw` → **`overseer-jdo`** | **jcw is CLOSED** (2026-07-30T22:24Z, as a DUPLICATE of `overseer-jdo`; re-measured 2026-07-31T14:40Z). jdo is P1, open, and the single live home. Mechanism 1 (shared tmux socket) FIXED and gated. Mechanism 2 COSTED here — but **jdo's notes predate both the severity correction and the costing**, see "WHAT jdo IS MISSING" below. **Needs a contract decision** — but **2026-08-01 ADDS A SEQUENCING CONSTRAINT THAT COMES FIRST**: jdo's acceptance bar is 20 consecutive clean runs, and clean runs are counted from a board with a DEMONSTRATED blind mode for the very test being counted (`test_both_forms_report_busy_while_a_pane_keeps_changing` has asserts at statements 4 and 5 of 6 — failing at the last leaves nothing unexecuted, so coverage holds and the board goes green). **The denominator is unsound until `check-per-file-coverage` is unmasked, so the order is: unmask, then measure, then decide the contract.** |
...
| 3 | **A recurring CHECK instead of a sweep** — `tests/prompts/test_charters_carry_no_known_defects.py` (NOT the gitignored `blast_radius.py` prototype it was costed against) runs in each repo's CI | needs the tool productionised AND adopted per repo | **Half discharged.** The tool IS productionised, eleven classes, green here, and imports and runs unmodified from outside this repo. What remains is per-repo ADOPTION, not construction. **But it reaches only 94 of 117** — `homelab` consumes no pin. **AND 2026-08-01 ADDS A SECOND OBSTACLE ON THE SAME REPO:** the (h) detector hardcodes `with-livespec-env.sh`, so adopting this gate into `homelab` flags homelab's CORRECT `with-homelab-env.sh -- bd show` as a defect — a FALSE POSITIVE that fails quietly, unlike the `_REPO_ROOT` caveat which reddens loudly. **Parameterising the wrapper name is a precondition for the "+1" half of the recommended 3+1 shape.** Also: 1 of homelab's 23 is that false positive, so the fleet's real figure is **116, not 117**. |
...
- **`.ai/supervisor-protocol.md`** — the shared role layer, holding **24**
  role-level Corrections, C1–C24. Re-counted 2026-08-03 (847 lines).
