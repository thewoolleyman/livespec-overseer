<!--
PROVENANCE. This note was authored 2026-08-15 for a plan thread
`foreman-missing-session-commands` that was never opened. It arrived as PR #935,
which would have created a SIXTH plan directory. The 2026-08-19 grooming pass
folded its anchor epic `overseer-7jskz4` into THIS thread instead, with
`overseer-4bbnit` as its implementing child, so the research is preserved here
and PR #935 was closed as superseded rather than merged.

Read it as write-once research, exactly as its own header says: it is never
authoritative about what remains, and its module paths were measured 2026-08-15
and must be re-measured before being trusted.
-->
