...
> >
> > > # ✅ THE PROHIBITION BELOW IS RETIRED — 2026-08-02. `overseer-wr8` LANDED. Read this before obeying it.
> > >
> > > The paragraph that follows says the charter must NOT be regenerated, and it was
> > > correct WHEN WRITTEN. It is now false, and left standing it permanently blocks
> > > the very repair it was protecting — a "do not fix this" note that outlives its
> > > cause becomes the defect.
> > >
> > > **`overseer-wr8` landed (PR #403).** Verified against **`origin/master`**, not a
> > > local checkout, with controls: `.ai/supervisor-protocol.md` now returns
> > > `empty result is not a finding` **1**, `wait is not a question` **1**,
> > > `positive control` **2**, against `armed re-entry` **1** as a positive control
> > > and a bogus phrase at **0**. The two rule sections are promoted into the SHARED
> > > layer, so regeneration no longer deletes them.
> > >
> > > **So: regeneration is SAFE again, and the bind is resolved.** Regenerating
> > > remains the supervisor's act rather than the worker's, and whoever does it
> > > should still diff the output against the current file before accepting it —
> > > that caution was never about wr8.
> > >
> > > **The mechanism below is KEPT, because it is a good trap description**: a
> > > generator that preserves byte-for-byte only from one heading will silently drop
> > > standalone `##` rule sections outside it, and nothing else was gating them.
> > > Read it as a hazard that HAS BEEN CLOSED, not as a live instruction.
> >
> > **The charter WAS in a genuine BIND, and naming it is the point.** It was not
...
> > the contract test exists to prevent. And at the time it must not have been
> > REGENERATED either, because regeneration deleted two rule sections. **Neither
> > lever was available.** Until `overseer-wr8` landed, this file was authoritative
> > over the charter on every row in the table above, and the charter's *role-level
> > rules* were binding — they were the part regeneration would have destroyed,
> > which is exactly why they were worth keeping. **All of that is now past tense.**
> >
...
the gate honestly rather than by fabrication. It was filed as **`overseer-2a1`**
and **the defect is now FIXED** — the preconditions are split by phase, so the
workaround below is retired rather than carried forward.
...
**How the charter was obtained here:** the supervised and supervisor tmux
sessions were started first, with the supervised pane's cwd inside
`/data/projects/livespec-overseer` and a live agent driver in it, and the skill
was then re-run. That satisfied all five preconditions honestly rather than by
fabrication.

**DO NOT REPEAT THAT ON THE NEXT THREAD — the ordering it worked around is
RETIRED by `overseer-2a1`.** The maintainer ratified the precondition split on
2026-08-23: the artifact check (`plan-directory-exists`) gates AUTHORING, and
the four live-session checks gate DRIVING. A thread with no tmux sessions at all
is now charterable, so run `/livespec-overseer:supervise-plan` directly at
thread creation. Standing up two sessions in order to author a charter is
exactly the inversion that was fixed, and following the retired recipe would
restore it by hand. The four live-session checks are unchanged and still HALT at
drive time with the same messages and the same expected names. **Do not
hand-write the file** — a hand-written charter is exactly the evidence-free
artifact the generated-charter contract test exists to prevent.
