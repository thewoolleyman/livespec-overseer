...
def _evaluate_ledger_invariants(
    *,
    module: GroomingConformanceModule,
    repo: Path,
    work_items: list[dict[str, object]],
    seat_anchor_epic_ids: set[str] | None = None,
) -> ReportView:
    try:
        return module.evaluate_ledger_invariants(
            repo=repo,
            work_items=work_items,
            seat_anchor_epic_ids=seat_anchor_epic_ids,
        )
    except TypeError as exc:
        if "seat_anchor_epic_ids" not in str(exc):
            raise
        return module.evaluate_ledger_invariants(repo=repo, work_items=work_items)
...
def test_reports_breaching_item_ids_and_unimplemented_invariants(*, tmp_path: Path) -> None:
    ...
    report = _evaluate_ledger_invariants(
        module=module,
        repo=repo,
        work_items=[
