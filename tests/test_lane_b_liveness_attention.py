...
def test_ready_downgrade_leaves_missing_state_armed(*, tmp_path):
    repo, topic = make_plan(tmp_path=tmp_path)
    session = registry.tmux_id(repo=str(repo), topic=topic)
...
def test_ready_downgrade_write_failure_is_logged_and_leaves_ready(*, tmp_path, monkeypatch):
    repo, topic = make_plan(tmp_path=tmp_path)
    session = registry.tmux_id(repo=str(repo), topic=topic)
    ...
    def write_raises(self, data, *, encoding=None, errors=None):
        raise OSError("simulated write failure")

    monkeypatch.setattr(signals.Path, "write_text", write_raises)
    ...
    assert ready is True
    assert "could not downgrade ready declaration" in err.getvalue()
