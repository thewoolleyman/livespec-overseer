...
def test_governed_typed_ruling_vocabulary_is_empty():
    typed_ruling = foreman_typed_ruling()

    assert typed_ruling.governed_ruling_kinds() == frozenset()
    assert typed_ruling.ruling_kind_defined(kind="relay-to-session") is False
...
def journal_document(
    *, repo: Path, records: list[dict[str, object]], records_read: int | None = None
) -> dict[str, object]:
    document = base_document(repo=repo)
    document["sources"] = {
        **document["sources"],
        "dispatch_journal": {
            ...
            "records_read": records_read if records_read is not None else len(records),
        },
    }
...
def host_published_reconcile_proposal(
    *, repo: Path, record: dict[str, object], records_read: int = 1
) -> dict[str, object]:
...
def test_typed_work_item_filing_uses_intake_seam_and_journals_before_result(*, tmp_path):
    ...
    journaled: list[tuple[Path, dict[str, object]]] = []
    events: list[str] = []
    pending_record = {
        "stage": "foreman-act",
    ...
    def file_item(*, request: dict[str, object]):
        ...
        events.append("file")
        assert journaled == [(repo, pending_record)]
        return "overseer-new", "ready"

    def append_record(*, repo: Path, record: dict[str, object]) -> None:
        journaled.append((repo, record))
        events.append(f"journal:{record['reason']}")

    result = module.act(
        proposal=file_proposal(repo=repo, target_repo=target_repo),
        seams=module.ActSeams(
            gather=lambda *, repo, snapshot_path: base_document(repo=Path(repo)),
            run=lambda *, argv: 99,
    ...
    assert journaled[0] == (repo, pending_record)
    assert journaled[-1] == (
        repo,
        {
            "stage": "foreman-act",
...
def test_work_item_filing_refuses_when_pending_journal_append_fails(*, tmp_path):
    ...
    repo_path = repo
    result = module.act(
        proposal=file_proposal(repo=repo_path),
        seams=module.ActSeams(
            gather=lambda *, repo, snapshot_path: base_document(repo=Path(repo)),
            run=lambda *, argv: 99,
            file_work_item=file_item,
...
def test_malformed_work_item_filing_returns_failed_result(*, tmp_path):
    ...
    def malformed_filing(*, request: dict[str, object]):
        ...
        assert request["target_repo"] == str(target_repo)
    ...
    result = module.act(
        ...
        seams=module.ActSeams(
            ...
            append_journal=lambda *, repo, record: journaled.append((repo, record)),
        ),
    )
...
def test_host_published_failed_outcome_reconciles_from_forge_evidence(*, tmp_path):
    ...
    result = module.act(
        proposal=host_published_reconcile_proposal(repo=repo, record=records[1], records_read=2),
        seams=module.ActSeams(
            gather=lambda *, repo, snapshot_path: journal_document(
                repo=Path(repo), records=records
            ),
...
def test_reconcile_refuses_aged_out_record_as_outside_gather_window(*, tmp_path):
    module = foreman_act()
    repo = tmp_path / "repo"
    record = journal_record()
    proposal = reconcile_proposal(repo=repo, record=record)
    ...
    result = module.act(
        ...
        seams=module.ActSeams(
            gather=lambda *, repo, snapshot_path: journal_document(
                repo=Path(repo), records=[], records_read=3
            ),
            run=lambda *, argv: calls.append(argv) or 0,
            append_journal=lambda *, repo, record: None,
        ),
    ...
    assert result["outcome"] == "refused"
    assert result["reason"] == "journal_record_outside_gather_window"
    assert calls == []


def test_reconcile_still_names_changed_record_inside_gather_window(*, tmp_path):
    module = foreman_act()
    repo = tmp_path / "repo"
    proposed_record = journal_record()
    gathered_record = journal_record(work_item_id="overseer-other")
    proposal = reconcile_proposal(repo=repo, record=proposed_record)
    calls: list[list[str]] = []
    ...
    result = module.act(
        ...
        seams=module.ActSeams(
            gather=lambda *, repo, snapshot_path: journal_document(
                repo=Path(repo), records=[gathered_record], records_read=1
            ),
            run=lambda *, argv: calls.append(argv) or 0,
            append_journal=lambda *, repo, record: None,
        ),
    ...
    assert result["outcome"] == "refused"
    assert result["reason"] == "journal_record_changed"
    assert calls == []


def test_reconcile_generation_guard_refuses_real_journal_advance(*, tmp_path):
    module = foreman_act()
    repo = tmp_path / "repo"
    dispatcher = repo / ".orchestrator" / "bin" / "dispatcher.py"
    dispatcher.parent.mkdir(parents=True)
    ...
    result = module.act(
        ...
        seams=module.ActSeams(
            gather=lambda *, repo, snapshot_path: journal_document(
                repo=Path(repo), records=records, records_read=2
            ),
            run=lambda *, argv: calls.append(argv) or 0,
            append_journal=lambda *, repo, record: None,
        ),
    ...
    assert result["outcome"] == "refused"
    assert result["reason"] == "journal_generation_changed"
    assert calls == []


def test_reconcile_generation_guard_admits_when_journal_has_not_advanced(*, tmp_path):
    module = foreman_act()
    repo = tmp_path / "repo"
    dispatcher = repo / ".orchestrator" / "bin" / "dispatcher.py"
    dispatcher.parent.mkdir(parents=True)
    ...
    result = module.act(
        ...
        seams=module.ActSeams(
            gather=lambda *, repo, snapshot_path: journal_document(
                repo=Path(repo), records=[record], records_read=1
            ),
            run=lambda *, argv: calls.append(argv) or 0,
            append_journal=lambda *, repo, record: None,
        ),
...
def test_typed_ruling_escalates_while_governing_vocabulary_is_empty(*, tmp_path, monkeypatch):
    ...
    result = foreman_act.act(
        ...
        seams=foreman_act.ActSeams(
            gather=lambda *, repo, snapshot_path: blocked_document(repo=Path(repo)),
            run=lambda *, argv: pytest.fail("vocabulary-empty ruling must not run commands"),
...
def test_typed_ruling_does_not_relax_hard_floors(*, tmp_path):
    foreman_act = module("foreman_act")
    repo = tmp_path / "repo"
    repo.mkdir()
    write_consensus_config(repo=repo)
...
def test_cardinal_restart_rule_has_no_consensus_actuator_authorization_path():
    act_types = module("foreman_act_types")

    for action_id in act_types.ACTION_IDS:
