"""Guard that this repo ships EXACTLY two operator skills, in every harness.
...
about a fourth seat reappearing, and nothing at all about the two surfaces that
are supposed to REMAIN. After the bucket-2 trim (console plan decision D5,
maintainer ruling 2026-09-06) livespec-overseer is the two-pane overseer daemon
plus the caam-anthropic-loop and nothing else, so the shipped surface is now a
...
# The whole shipped operator surface, as ruled by D5 bucket 1.
SHIPPED_SKILLS = ("caam-anthropic-loop", "overseer")
...
def test_the_claude_skills_tree_ships_exactly_the_two_operator_skills() -> None:
    assert _subdirectory_names(directory=PLUGIN / "skills") == set(SHIPPED_SKILLS)


def test_the_codex_skills_tree_ships_exactly_the_two_operator_skills() -> None:
...
def test_the_pi_skills_tree_ships_exactly_the_two_operator_skills() -> None:
...
def test_the_harness_neutral_prose_covers_exactly_the_two_operator_skills() -> None:
...
def test_every_manifest_names_both_surviving_operations() -> None:
    """The complement of the check above: the two that stay must be advertised."""
    missing = [
