...
def test_uncertifiable_ready_below_seat_notice_bound_is_not_delivered(*, tmp_path):
    """The pane notice is gated behind a named stranded-ready age bound."""
...
def test_uncertifiable_ready_seat_notice_is_report_only_and_once_per_declaration(*, tmp_path):
    """A stranded ready declaration is told to its own pane once, without authority."""
    notice_after = getattr(_supervisor_ready_alerts, "STRANDED_READY_NOTICE_AFTER", None)
    assert isinstance(notice_after, int | float)
    clock = {"t": 1000.0}
    repo, topic, _session, fake, sup, track, _state = _bare_ready_no_round_fixture(
        tmp_path=tmp_path, clock=clock
    )
    ...
    assert surfaced.status == "ready-uncertifiable"
    [notice] = fake.paste_texts()
    assert "REPORT-ONLY" in notice
    assert "authorizes no restart" in notice
    assert "ready cannot certify" in notice
    assert "no supervision round open" in notice
    assert "1h" in notice
    assert "overseer-declare ready" not in notice
    assert len(fake.paste_texts()) == 1


def test_uncertifiable_ready_seat_notice_rearms_when_declaration_changes(*, tmp_path):
    """A fresh stranded ready declaration can be reported after the prior one."""
    notice_after = getattr(_supervisor_ready_alerts, "STRANDED_READY_NOTICE_AFTER", None)
    assert isinstance(notice_after, int | float)
    clock = {"t": 1000.0}
    repo, topic, _session, fake, sup, track, state = _bare_ready_no_round_fixture(
        tmp_path=tmp_path, clock=clock
    )
