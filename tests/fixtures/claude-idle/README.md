...
| `2.1.247.txt` | Live capture from the canonical capture path in the primary checkout, same provenance as `2.1.246.txt`. Captured 2026-08-27 for the same reason and by the same recipe: the installed build had moved again and no fixture existed for it, so `check-claude-idle-canary` failed on every operator host running it and no local push could pass the aggregate. This is the SECOND consecutive build to need this within a day; the registry gap reopens on every Claude Code update, which is a standing cost rather than an incident. |
| `2.1.251.txt` | Live capture from the canonical capture path in the primary checkout, same provenance and same reason as `2.1.246.txt` and `2.1.247.txt`: the installed build had moved on again (2.1.247 -> 2.1.251) and no fixture existed for it, so `check-claude-idle-canary` failed on every operator host and no local push could pass the pre-push aggregate. Captured 2026-08-29. Third instance in four days, which is the standing cost the `2.1.247.txt` row predicted rather than a new incident. |
